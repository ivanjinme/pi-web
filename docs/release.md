...
- npm package: `@weclio/pi-web`
- GitHub Release: `ivanjinme/pi-web`
...
- npm is authenticated as an account that can publish `@weclio/pi-web`.
...
- If `npm view @weclio/pi-web version` briefly shows the previous version, check the exact version instead:

```bash
npm view @weclio/pi-web@<version> version --registry https://registry.npmjs.org/
npm view @weclio/pi-web versions --json --registry https://registry.npmjs.org/
```
...
gh release view v<version> --repo ivanjinme/pi-web
```
...
- 发布 npm 包 `@weclio/pi-web@<version>`。
...
- Published npm package `@weclio/pi-web@<version>`.
```
...
```bash
gh release edit v<version> --repo ivanjinme/pi-web --notes-file - <<'EOF'
## 中文
